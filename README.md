# 🌑 Wasteds  
### Dark Low-Poly Open-World Survival RPG

![Status](https://img.shields.io/badge/Status-In_Development-blueviolet?style=for-the-badge)
![Engine](https://img.shields.io/badge/Engine-Godot_4-478CBF?style=for-the-badge&logo=godot-engine)
![Genre](https://img.shields.io/badge/Genre-Survival_RPG-ff4655?style=for-the-badge)
![Art](https://img.shields.io/badge/Art-Low--Poly_Realistic-8a7aff?style=for-the-badge)

<p align="center">
  <img src="Wasted.png" width="80%" alt="Wasteds banner (coming soon)"/>
</p>

---

<details open>
<summary><strong>🧭 About the Game</strong></summary>

<br>

**Wasteds** is a **third-person open-world survival RPG** set in a harsh, procedurally generated world where survival is never guaranteed.

Each new game creates a **unique land** filled with:
- Mysterious ruins 🏛
- Hostile magical creatures 👁
- Dangerous biomes and environmental threats 🌪

You begin alone.  
No map knowledge. No safety.  
Only your choices define the story.

</details>

---

<details>
<summary><strong>⚔ Core Features</strong></summary>

<br>

### 🌍 Procedural Open World
- Chunk-based terrain generation  
- Biomes, noise pipelines, erosion & height layers  

### 🧝 Character & Equipment
- Full character creation  
- Visible armor & weapons  
- NPCs share the same systems  

### 🔮 Risky Magic System
- Powerful spells with real consequences  
- High risk, high reward  

### 🧠 Hybrid Progression
- Attributes + skill trees  
- Inspired by Skyrim & World of Warcraft  

### 🎨 Visual Style
- Low-poly geometry  
- Hand-painted textures  
- Dark, atmospheric tone  

</details>

---

<details>
<summary><strong>🛠 Tech Overview</strong></summary>

<br>

- **Engine:** Godot 4.x  
- **Language:** GDScript  
- **World:** Chunk-based terrain generation  
- **Vegetation:** Rule-based procedural spawning  
- **Rendering:** Custom terrain shader  

Modular, system-driven architecture.

</details>

---

<details>
<summary><strong>🚧 Development Status</strong></summary>

<br>

- ✅ Terrain generation pipeline  
- ✅ Biomes & noise configs  
- ✅ Vegetation system  
- 🚧 Combat & magic  
- 🚧 AI & world interaction  

Expect breaking changes.

</details>

---

<details>
<summary><strong>📸 Media</strong></summary>

<br>

Coming soon:
- World generation previews  
- Terrain & biome screenshots  
- Gameplay clips  

</details>

---

<details>
<summary><strong>🤝 Contributing</strong></summary>

<br>

Contributions and feedback are welcome:
- Bug reports
- Performance improvements
- Gameplay ideas

Open an issue or discussion.

</details>

---

<details>
<summary><strong>📬 Contact</strong></summary>

<br>

- **GitHub:** https://github.com/Wasttedx  
- **Email:** Wastted@proton.me  

</details>

---

<details>
<summary><strong>⭐ Support</strong></summary>

<br>

If this project interests you:
- ⭐ Star the repo  
- 👀 Watch for updates  

It really helps.

</details>
